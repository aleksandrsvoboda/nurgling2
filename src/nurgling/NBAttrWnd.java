...
public class NBAttrWnd extends BAttrWnd {
	...
	public NAttr(Glob glob, String attr, Color bg) {
	    super(Coord.of(attrw, UI.scale(26)), glob, attr, bg);
	}
	...
	public NFoodMeter() {
	    super(Coord.of(attrw, UI.scale(24)));
	}
	...
	attrs = new ArrayList<>();
	NAttr aw;
	attrs.add(aw = add(new NAttr(glob, "str", every), prev.pos("bl").add(0, UI.scale(10) - catfDescent).add(wbox.btloff())));
	...
	prev = add(CharWnd.settip(new Img(catf.render(L10n.get("char.battr.fep")).tex()), "gfx/hud/chr/tips/fep"),
		   lframe.pos("bl").x(leftColX).add(0, UI.scale(10)));
	...
	prev = add(CharWnd.settip(new Img(catf.render(L10n.get("char.battr.hunger")).tex()), "gfx/hud/chr/tips/hunger"),
		   rframe.pos("bl").x(rightColX).add(0, UI.scale(10)));
