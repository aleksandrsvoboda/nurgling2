...
//        NoneStackableCurio.add(new JSONObject("{\"static\":\"gfx/invobjs/flotsam\",\"name\":\"Peculiar Flotsam\"}"));
    ...
    // Products confirmed (via the -yester icon entries elsewhere in this file) to actually spawn a
    // "Yesteryear's " variant in-game. For these, the normal and Yesteryear's pickup are tracked as two
    // separate LP discoveries instead of being merged into one. Everything else (catkins, pods, samaras,
    // nuts, etc.) isn't confirmed to have a Yesteryear variant, so it keeps the old merged behavior.
    private static final java.util.Set<String> yesteryearCapableProducts = new java.util.HashSet<>(java.util.Arrays.asList(
    ...
    // instead of the normal in-season product name; strip it to test membership against the object list,
    ...
    private static int expectedDiscoveryCount(String gobResName) {
        ArrayList<String> products = object.get(gobResName);
    ...
    public static boolean hasUndiscoveredProduct(String gobResName) {
        if (gobResName == null || !gobResName.startsWith("gfx/terobjs") || !object.containsKey(gobResName)
            || NUtils.getGameUI() == null || NUtils.getGameUI().getCharInfo() == null) {
            return false;
        }
        nurgling.widgets.NCharacterInfo info = NUtils.getGameUI().getCharInfo();
        for (String product : object.get(gobResName)) {
            if (yesteryearCapableProducts.contains(product)) {
