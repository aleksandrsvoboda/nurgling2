...
public class ConveyorProcessBuffers implements Action {
    ...
    private void updateOrdersAfterCheeseMovement(String cheeseType, int movedCount, CheeseBranch.Place fromPlace) {
        ConveyorOrder order = ordersManager.orderFor(cheeseType, fromPlace);
        if (order == null) {
            return;
        }
        order.advance(cheeseType, fromPlace, movedCount);
        ordersNeedSaving = true;
    }

    /**
     * Where the order these trays belong to sends them next, or null when no conveyor order wants them.
     */
    private CheeseBranch.Place getCorrectNextStageLocation(String cheeseType, CheeseBranch.Place currentPlace) {
        ConveyorOrder order = ordersManager.orderFor(cheeseType, currentPlace);
        if (order == null) {
            return null;
        }
        CheeseBranch.Cheese next = order.nextStep(cheeseType, currentPlace);
        return next != null ? next.place : null;
    }
