...
public class BuildCheeseRack implements Action {
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        try {
            ...
            SelectAreaWithLiveGhosts buildarea = new SelectAreaWithLiveGhosts(context, Resource.loadsimg("baubles/buildArea"), "Cheese Rack");
            buildarea.run(NUtils.getGameUI());

            // Use BuildMaterialHelper for auto-zone lookup
            BuildMaterialHelper helper = new BuildMaterialHelper(context, gui);
            command.ingredients.add(helper.getBoards(6));
