...
public class HempFarmer implements Action {
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        ...
        nContext.getSpecArea(Specialisation.SpecName.crop, "Hemp");
        ...
        if (new Validator(req, opt).run(gui).IsSuccess()) {
            ...
            if ((Boolean) NConfig.get(NConfig.Key.validateAllCropsBeforeHarvest)) {
                if (!new ValidateAllCropsReady(NContext.findSpec(field), new NAlias("plants/hemp")).run(gui).isSuccess) {
                    NUtils.stackSwitch(oldStackingValue);
                    gui.msg("Not all hemp crops are ready for harvest, skipping harvest.");
