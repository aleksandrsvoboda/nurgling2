...
public class ISlots extends ItemInfo.Tip implements GItem.NumberInfo {
	...
	if(!(owner instanceof NGItem) || ((NGItem)owner).ui.modshift) {
		...
		if(!mods.isEmpty())
		{
		l.cmp.add(AttrMod.modimg(mods.values()), new Coord(UI.scale(10), l.cmp.sz.y));
		l.cmp.add(RichText.render("$col[168,168,168]{[Press SHIFT for details]}", 0).img, new Coord(UI.scale(10), l.cmp.sz.y));
		}
	}
	if(left > 0)
